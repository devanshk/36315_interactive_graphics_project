...
census <- read_csv("/Users/devanshkukreja/Box Sync/CMU 2018 Spring/Stat Vis - 36315/36315_interactive_graphics_project/dkukreja/adult_census_income.csv") %>% 
  select(-fnlwgt) %>% 
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Ross", tabName = "ross", icon = icon("dashboard")),
      menuItem("Devansh 1", tabName = "devansh_1", icon = icon("th")),
      menuItem("Devansh 2", tabName = "devansh_2", icon = icon("th")),
      menuItem("Oliver 1", tabName = "oliver_1", icon = icon("th")),
      menuItem("Oliver 2", tabName = "oliver_2", icon = icon("th"))
    )
  ),
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "ross",
        fluidRow(
          ...
          box(plotOutput("inc_plot", width="100%"), width=8),
